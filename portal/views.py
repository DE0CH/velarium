...
def install_keys(request):
    ...
    if user and not user.registered and request.method == 'POST':
        ...
        if not settings.DEMO:
            ...
            p = subprocess.run(f'{os.path.join(settings.BASE_DIR, "make-user.sh")} {user.username}', shell=True)
            p = subprocess.run(f'sudo -u {user.username} {os.path.join(settings.BASE_DIR, "install-keys.sh")} {github_username}', shell=True)
            if p.returncode != 0:
